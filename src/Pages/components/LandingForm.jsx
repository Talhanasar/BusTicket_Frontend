...
const LandingForm = () => {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col items-center gap-4 p-6 rounded-lg shadow-lg bg-white bg-opacity-90 w-full max-w-md">
            <h2 className="text-xl font-bold text-[#1DD100]">Book Your Bus</h2>

            <div className='flex gap-3'>
                <div className="w-full relative">
                    <label className="block mb-2 text-sm font-medium text-[#1DD100CC]">From</label>
                    <input
                        autoComplete='off'
                        ...
                        type="text"
                        className="w-full p-3 border rounded-md text-[#030712] focus:outline-none focus:ring-1 focus:ring-[#1dd1004d] focus:shadow-[0_0_10px_4px_rgba(29,209,0,0.6)] border-[#1DD10080]"
                        placeholder="Enter departure city"
                    />
                    {showSuggestionsFor === 'from' && suggestions.length > 0 && (
                        <div className="absolute z-10 w-full bg-white border border-[#1DD10080] rounded-md shadow-lg mt-2 max-h-[10rem]">
                ...
                <div className="w-full relative">
                    <label className="block mb-2 text-sm font-medium text-[#1DD100CC]">To</label>
                    <input
                        onChange={handleInputChange}
                        onFocus={handleInputFocus}
                        autoComplete='off'
                    ...
                    {showSuggestionsFor === 'to' && suggestions.length > 0 && (
                        <div className="absolute z-10 w-full bg-white border border-[#1DD10080] rounded-md shadow-lg mt-2">
                            {suggestions.map((suggestion, index) => (
                                <div
                                    key={index}
                                    className="p-2 cursor-pointer hover:bg-[#1DD10033]"
            ...
            <div className="w-full">
                <label className="block mb-2 text-sm font-medium text-[#1DD100CC]">Date</label>
                <input
                    onChange={handleInputChange}
                    name='date'
                    value={placeAndDate.date}
                    type="date"
                    className="custom-date-input w-full p-3 border rounded-md text-[#030712] focus:outline-none focus:ring-1 focus:ring-[#1dd1004d] focus:shadow-[0_0_10px_4px_rgba(29,209,0,0.6)] border-[#1DD10080]"
                />
            </div>

            <button type="submit" className="w-full py-3 mt-4 text-white bg-[#1DD100] rounded-md hover:bg-[#1DD100CC] transition-all">
                Search Buses
            </button>
        </form>
    );
