...
const FormRight = ({ selectedSeats, unitPrice, coupons, busId }) => {
    ...
    const handleApplyCoupon = () => {
        ...
        const preDiscountTotal = selectedSeats.length * unitPrice;
        const foundCoupon = coupons.find(coupon => coupon.code === inputCoupon.value);
        if (foundCoupon) {
            if (preDiscountTotal < Number(foundCoupon.minPurchaseAmount || 0)) {
                return toast.error(`Coupon requires minimum purchase of ${foundCoupon.minPurchaseAmount}`);
            }
            ...
            <div className="flex flex-col font-['Inter'] text-[#030712]">
                ...
                <div className="rounded-xl bg-[#F7F8F8] p-[1.7em] m-[1em] w-full max-w-lg mx-auto">
                    ...
                    <div className='max-h-[15rem] min-h-[4rem] overflow-auto'>
                        {selectedSeats.map((seat, idx) => (
                            <div key={idx} className="flex justify-between w-full text-[rgba(3,7,18,0.2)] text-[0.8em] mb-[0.8em]">
                                <span>{seat}</span>
                                <span>{unitPrice}</span>
                            </div>
                        ))}
                    ...
                    <div className="bg-white rounded-[0.75em] p-[0.3em] my-[0.7em] flex justify-between items-center">
                        ...
                        <button
                            type="button"
                            disabled={!inputCoupon.value.length}
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-[1em] w-full">
                    {['Passenger Name*', 'Phone Number*', 'Email ID'].map((label, idx) => {
                        ...
                        return (
                            <div key={idx}>
                                <label className="font-semibold text-[0.8em]" htmlFor={`input-${idx}`}>{label}</label>
                                <input
                                    id={`input-${idx}`}
                                    type="text"
                    ...
                    <button type='submit'
                        ...
                        className="bg-[#1DD100] rounded-[1em] w-full max-w-lg mx-auto py-[0.8em] text-white font-extrabold text-[1em] font-['Raleway'] my-[1.2em] disabled:opacity-60 disabled:cursor-not-allowed">
